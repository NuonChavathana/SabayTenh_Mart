...
import { db, productsTable, categoriesTable, inventoryLogsTable, eq, lte } from "@workspace/db";
...
router.get("/", requireAuth, requireRole("staff", "cashier", "admin"), async (req, res): Promise<void> => {
...
router.put("/:productId", requireAuth, requireRole("staff", "admin"), async (req, res): Promise<void> => {
  ...
  const updates: Partial<Pick<Product, "stock" | "lowStockThreshold">> = {
