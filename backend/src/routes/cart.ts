...
import { db, cartsTable, cartItemsTable, productsTable, eq, and } from "@workspace/db";
...
async function getOrCreateCart(userId: number) {
  ...
  if (!cart) {
    const [inserted] = await db.insert(cartsTable).values({ userId }).$returningId();
    const [newCart] = await db.select().from(cartsTable).where(eq(cartsTable.id, inserted.id));
    cart = newCart;
  }
  return cart;
}
...
async function buildCartResponse(cartId: number) {
  ...
  originalPrice: item.originalPrice
    ? parseFloat(String(item.originalPrice))
    : null,
  ...
  const originalPrice =
    i.originalPrice && Number(i.originalPrice) > price
...
router.put("/items/:itemId", requireAuth, async (req, res): Promise<void> => {
...
router.delete("/items/:itemId", requireAuth, async (req, res): Promise<void> => {
  const raw = Array.isArray(req.params.itemId) ? req.params.itemId[0] : req.params.itemId;
  const itemId = parseInt(raw, 10);
  if (isNaN(itemId)) { res.status(400).json({ error: "Invalid itemId" }); return; }
  const cart = await getOrCreateCart(req.user!.userId);
...
router.delete("/clear", requireAuth, async (req, res): Promise<void> => {
  const cart = await getOrCreateCart(req.user!.userId);
