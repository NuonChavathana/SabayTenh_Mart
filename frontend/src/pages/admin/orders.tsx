    ...
    <div className="bg-muted/30 dark:bg-muted/10 border-t px-4 py-5">
      <div className="grid md:grid-cols-2 gap-5">
        ...
        <div className="space-y-6">
          ...
          {/* Added Order Items list with the requested image helper replacement */}
          {order.items && order.items.length > 0 && (
            <div>
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">
                {t("Order Items", "ទំនិញដែលបានកុម្មង់")}
              </p>
              <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                {order.items.map((item: any, idx: number) => (
                  <div key={idx} className="flex items-center justify-between border rounded-xl p-2 bg-white dark:bg-card text-sm">
                    <div className="flex items-center gap-3">
                      {item.product?.image ? (
                        ...
                        <div className="w-10 h-10 rounded-md border bg-muted flex items-center justify-center text-muted-foreground text-[10px]">No Pic</div>
                      )}
                      <div>
                    ...
                    <p className="font-semibold text-primary">${Number(item.price * item.quantity).toFixed(2)}</p>
                  </div>
                ))}
              </div>
            </div>
...
export default function AdminOrdersPage() {
  ...
  const filtered = (orders ?? []).filter((o: any) => {
    ...
    const customerName = o.customerName ?? o.userName ?? o.user?.name ?? "";
    const matchesSearch = !search || String(o.id).includes(q) || customerName.toLowerCase().includes(q);
