...
function saveAddresses(a: Address[]) { localStorage.setItem(ADDR_KEY, JSON.stringify(a)); }
function getSavedPayments(): SavedPayment[] {
  try { return JSON.parse(localStorage.getItem(PM_KEY) ?? "[]"); } catch { return []; }
}
function saveSavedPayments(p: SavedPayment[]) { localStorage.setItem(PM_KEY, JSON.stringify(p)); }
  ...
  const update = useUpdateProfile({
    mutation: {
      onSuccess: (u) => {
        ...
        setEditing(false);
        toast({ title: t("Profile updated!", "ព័ត៌មានបានធ្វើបច្ចុប្បន្នភាព!") });
      },
      onError: () => toast({ title: t("Failed to update.", "មិនអាចធ្វើបច្ចុប្បន្នភាព"), variant: "destructive" }),
    },
  });

  const roleLabel: [string, string] = ({ admin: ["Administrator","អ្នកគ្រប់គ្រង"], staff: ["Staff","បុគ្គលិក"], cashier: ["Cashier","គណនេយ្យករ"], customer: ["Customer","អតិថិជន"] } as Record<string,[string,string]>)[user?.role ?? "customer"] ?? ["Customer","អតិថិជន"];
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-card border rounded-2xl p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold">{t("Personal Information", "ព័ត៌មានផ្ទាល់ខ្លួន")}</h3>
          {!editing && (
            <Button variant="ghost" size="sm" className="gap-1.5 text-xs h-8" onClick={() => { setName(user?.name ?? ""); setPhone(user?.phone ?? ""); setEditing(true); }}>
        ...
        {editing ? (
          <div className="space-y-3">
            <div>
              ...
              <Input value={name} onChange={e => setName(e.target.value)} className="mt-1 h-9" placeholder={t("Your name", "ឈ្មោះ")} />
            </div>
            <div>
              <Label className="text-xs">{t("Phone", "ទូរស័ព្ទ")}</Label>
              <Input value={phone} onChange={e => setPhone(e.target.value)} className="mt-1 h-9" placeholder="0xx xxx xxx" />
            </div>
            <div className="flex gap-2 pt-1">
              <Button size="sm" onClick={() => update.mutate({ data: { name: name.trim(), phone: phone.trim() || undefined } })} disabled={update.isPending} className="gap-1.5">
                <Check className="h-3.5 w-3.5" />{update.isPending ? t("Saving...", "...") : t("Save", "រក្សាទុក")}
              </Button>
          ...
          <div className="space-y-3 text-sm">
            <div className="flex items-center gap-3"><User className="h-4 w-4 text-muted-foreground" /><span>{user?.name ?? "—"}</span></div>
            ...
            <div className="flex items-center gap-3">
              ...
              <span>{user?.phone ?? <span className="text-muted-foreground italic">{t("Not set", "មិនទាន់")}</span>}</span>
            </div>
            <div className="flex items-center gap-3">
              <Shield className="h-4 w-4 text-muted-foreground" />
              <span className="text-muted-foreground">{t("Member since", "សមាជិកចាប់ពី")} {user?.createdAt ? new Date(user.createdAt).toLocaleDateString() : "—"}</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {([
          { label: t("Total Orders", "ការបញ្ជា"), icon: Package, color: "text-primary bg-primary/10" },
          { label: t("Wishlist Items", "ចង់ទិញ"), icon: Heart,   color: "text-rose-500 bg-rose-50 dark:bg-rose-900/20" },
          { label: t("Reviews Given", "មតិ"),     icon: Star,    color: "text-yellow-500 bg-yellow-50 dark:bg-yellow-900/20" },
        ] as const).map((s, i) => (
      ...
      {(wishlist as any[]).map(item => (
        <div key={item.id} className="bg-white dark:bg-card border rounded-xl overflow-hidden hover:shadow-md transition-shadow group">
          <Link href={`/products/${item.productId}`} className="relative block aspect-square overflow-hidden bg-muted">
            <img src={item.productImage ?? "https://images.unsplash.com/photo-1553062407-98eeb64c6a62?w=300"} alt={item.productName} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
            {item.discountPercent && <Badge className="absolute top-2 left-2 bg-destructive text-white border-0 text-[10px]">-{item.discountPercent}%</Badge>}
          </Link>
          <div className="p-2.5">
...
function AddressesSection() {
  ...
  const commit = (list: Address[]) => { setAddresses(list); saveAddresses(list); };
...
function PaymentsSection() {
  const { t } = useLanguage();
  const [saved, setSaved]             = useState<SavedPayment[]>(getSavedPayments);
  const [showForm, setShowForm]       = useState(false);
  ...
  const commit = (list: SavedPayment[]) => { setSaved(list); saveSavedPayments(list); };
