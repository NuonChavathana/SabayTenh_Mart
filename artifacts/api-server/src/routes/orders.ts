...
async function buildOrderRow(order: any, withItems = false) {
...
router.get("/orders", requireAuth, async (req, res): Promise<void> => {
  ...
  if (req.query.status) {
    conditions.push(eq(ordersTable.status, req.query.status as any));
  }
...
router.post("/orders", requireAuth, async (req, res): Promise<void> => {
  ...
  for (const item of cartItems) {
    await db.update(productsTable).set({ soldCount: sql`sold_count + ${item.quantity}` }).where(eq(productsTable.id, item.productId));
  }
...
router.put("/orders/:id/status", requireAuth, requireRole("cashier", "staff", "admin"), async (req, res): Promise<void> => {
  const raw = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
  const id = parseInt(raw, 10);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid id" }); return; }
